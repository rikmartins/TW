    ...
    async function () {
                ...
                (possibleNobleLandingTime) => {
                    Object.values(incomingsObject).forEach((incoming) => {
                        ...
                        if (
                            ...
                            parseFloat(distance) <=
                                parseFloat(maxNobleTravelDistance)
                        ) {
            ...
            jQuery('#raWatchtowerTimerBtn').on('click', function (e) {
                ...
                if (parseInt(worldConfig.config.game.watchtower)) {
                    if (jQuery('#incomings_table tr:eq(0) th').length === 7) {
                        ...
                        $.getScript(
                            'https://www.dl.dropboxusercontent.com/s/w9gtfwq4d1obngj/watchtower.js'
                        );
                        console.debug(
        ...
        function sortHours(arr) {
            arr.sort((timeA, timeB) => {
                const [hoursA, minutesA, secondsA] = timeA
                    .split(':')
                    .map(Number);
                const [hoursB, minutesB, secondsB] = timeB
                    .split(':')
                    .map(Number);
